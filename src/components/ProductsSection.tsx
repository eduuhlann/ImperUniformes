...
import { COMPANY_INFO, UNIFORM_CATEGORIES } from '../data/imperData';
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
          {UNIFORM_CATEGORIES.map((cat, idx) => (
            <StaggerItem key={cat.name} index={idx}>
              <div className="group rounded-2xl overflow-hidden border-2 border-red-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm hover:shadow-xl hover:border-red-600 dark:hover:border-red-600 transition-all duration-300 flex flex-col">
                <div className="relative h-40 sm:h-48 overflow-hidden bg-red-100 dark:bg-zinc-800">
                  <img
                    src={cat.image}
                    alt={`Uniforme ${cat.name} Imper`}
                  ...
                  <button
                    type="button"
                    onClick={() => setSelectedImage({ src: cat.image, name: cat.name })}
                    ...
                    aria-label={`Ampliar imagem de ${cat.name}`}
                ...
                <div className="p-4 flex flex-col justify-between flex-1">
                  ...
                  <a
                    ...
                    className="mt-3 inline-flex items-center justify-center gap-1.5 text-[11px] sm:text-xs font-bold capitalize tracking-wider text-red-700 dark:text-red-400 border border-red-200 dark:border-red-900/60 bg-red-50/60 dark:bg-red-950/40 hover:bg-red-600 hover:text-white dark:hover:bg-red-600 dark:hover:text-white transition-colors px-3 py-2 rounded"
                  >
